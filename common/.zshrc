...
#Vim
alias vi='vim' #so i dont use the older version of vim.
export EDITOR=vim
...
tskr(){
    str="$*"
    tmux send-keys -t Right "$str" c-m;
}
tskl(){
    str="$*"
    tmux send-keys -t Left "$str" c-m;
}
tsku(){
    str="$*"
    tmux send-keys -t top "$str" c-m;
}
tskd(){
    str="$*"
